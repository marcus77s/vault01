# LLM Wiki Schema — vault01 (Gemini CLI)

## O que é este vault

Este vault é uma **LLM Wiki**: uma base de conhecimento persistente e interligada mantida pelo Gemini CLI (Antigravity). Marcus cuida da curadoria de fontes e faz as perguntas. Gemini CLI escreve e mantém todo o wiki — resumos, páginas de conceitos, sínteses, cross-references, log e índice.

**Regra fundamental**: arquivos em `Clippings/` e `RSS articles/` são **fontes brutas imutáveis**. Gemini CLI lê, nunca modifica. Tudo o que o Gemini CLI cria vai em `Knowledge/`.
...
Todo arquivo criado pelo Gemini CLI deve ter frontmatter YAML:
...
2. **Leia** as páginas identificadas in `Knowledge/`
...
5. **Conflitos vão para `Knowledge/Pending/`** — Marcus decide, não o Gemini CLI
